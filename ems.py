...
def run_ems():
    ...
    def search_employee():
        if searchEntry.get() == "":
            messagebox.showerror("Error", "Enter value for search")
    ...
    def update_employee():
        ...
        if not selected_item:
            ...
            database.update(idEntry.get(),nameEntry.get(),phoneEntry.get(),
                            roleBox.get(),genderBox.get(),salaryEntry.get())
            treeview_data()
            clear()
    ...
    def add_employee():
        if nameEntry.get() == "" or phoneEntry.get() == "" or roleBox.get() == "" or genderBox.get() == "" or salaryEntry.get() == "" or idEntry.get() == "":
            ...
            messagebox.showerror("Error", "Invalid Id Format Please Use (EMP) followed by a number (eg..EMP1)")
        else:
            database.insert(idEntry.get(),nameEntry.get(),phoneEntry.get(),
                            roleBox.get(),genderBox.get(),salaryEntry.get())
            treeview_data()
            clear()
    ...
    role_options=['Web Developer','Data Scientist','Ai Engineer','Network Engineer',
                  'IT Consultant','UI UX Designer','Cloud Architect','Technical Writer']
